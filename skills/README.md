...
Ask your agent to run or prepare a gate review, or to decide whether an AI initiative can advance: "run a stage-gate review", "are we ready for Gate 3?", "Go/Kill decision for this AI prototype", "should this take the fast track?". The skill activates on gate-decision requests and routes itself through `SKILL.md`; the depth lives in `references/` (gate-review workflow, the six stages and their gates, gate-decision logic, AI-specific validation, fast track, output templates).
