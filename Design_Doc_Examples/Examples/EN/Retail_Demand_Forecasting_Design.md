# Supermegaretail — Demand Forecasting System Design
...
*Starting from the following section of this design document, we've sketched questions only, to avoid this becoming too voluminous. Answering these questions will help decide on further actions, while the answers are revealed in the later sections as we pace through the different stages of the system.*
...
Reviewing a design document is just as important. See the review checklist in [`design_doc_checklist.md`](../../../templates/design_doc_checklist.md) for practical guidance on reviewing a design doc like this one.
...
The symmetric MAPE (sMAPE) is a 'symmetrized' version of the MAPE, which is defined if the point forecasts and actuals are not both zero at all future time points. However, in any period with a zero actual, its contribution is two, regardless of the point forecast, making it unsuitable for count data.
...
**Notation used in the quantile loss formulas:**
...
3.  (Optionally) We rewrite not only the last day but also the previous 2-3 days to fix possible data corruptions (duplicates, incomplete data, and so on).
...
For automatic feature selection during the first stages (when we haven't determined the basic feature set yet), we use RFE (recursive feature elimination).
...
- The general prediction quality steadily increases across all metrics, and across the majority of validation folds, when switching to a unified model.
...
**Error rates.** We set the significance level to 5% and the type II error to 10%.
...
- **Methodology** to be supplied in the appendix. E.g., how groups were picked to be representative of each other, simulations run to check for type I and type II errors, etc.
...
- **Secondary fallback.** Our next layer of fallback involves time-series models like SARIMA or Prophet, which we explored in section V.iv (Time series-specific baseline). These models are less dependent on external features, allowing for more robust predictions if drift occurs.
...
We will set up prediction drift monitoring. We will use it as an early alarm for the next day, week and month predictions. We will test two approaches: Population Stability Index > 0.2 and Wasserstein distance > 0.1. For Wasserstein distance, we will apply a growth multiplier to the control dataset. For example, when comparing April 2021 to April 2022, knowing that overall growth is expected to be 15% we will multiply everything in 2021 by 1.15. We will further adjust this based on historical data experimentation.
